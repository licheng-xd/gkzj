...
public class ApplicationSecurity extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
            .antMatchers("/", "/login", "/register", "/rule", "/faq")
            .permitAll()
            .antMatchers("/css/**", "/fonts/**", "/js/**", "/img/**")
            .permitAll()
            .antMatchers("/template.xlsx", "/manage", "/search", "/import",
                "/delete").hasRole("ADMIN")
            .anyRequest().authenticated()
            .and().formLogin().loginPage("/login").defaultSuccessUrl("/profile")
            .failureHandler(new UsernameStoringUrlAuthenticationFailureHandler(
                "/login?error"))
    ...
    private class UsernameStoringUrlAuthenticationFailureHandler
        extends SimpleUrlAuthenticationFailureHandler {
        UsernameStoringUrlAuthenticationFailureHandler(
        ...
        public void onAuthenticationFailure(HttpServletRequest request,
            ...
            request.getSession(true)
                .setAttribute("lastUsername", request.getParameter("username"));
